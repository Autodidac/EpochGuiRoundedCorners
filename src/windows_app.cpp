...
namespace
{
    ...
    struct DemoVisual
    {
        ...
        ComPtr<ID2D1PathGeometry> outer{};
        ComPtr<ID2D1PathGeometry> inner{};
    };

    [[nodiscard]] HRESULT create_geometry(
    ...
    public:
        [[nodiscard]] HRESULT initialize(HINSTANCE instance) noexcept
        {
            ...
            if (FAILED(result = create_text_formats()))
                return result;
            if (FAILED(result = create_visuals()))
                return result;
            ...
            RECT frame{ 0, 0, static_cast<LONG>(kCanvasWidth), static_cast<LONG>(kCanvasHeight) };
    ...
    private:
        ...
        [[nodiscard]] HRESULT create_visuals() noexcept
        {
            ...
            for (DemoVisual& visual : visuals_)
            {
                ...
                HRESULT result = create_geometry(
                    *factory_.Get(),
        ...
        [[nodiscard]] HRESULT ensure_render_target() noexcept
        {
            ...
            const D2D1_SIZE_U size{
                static_cast<UINT32>(client_width > 0 ? client_width : 1),
                static_cast<UINT32>(client_height > 0 ? client_height : 1)
            };
        ...
        void render() noexcept
        {
            ...
            {
                brush_->SetColor(visual.mesh.border_width > 0.0f ? visual.border : visual.fill);
                render_target_->FillGeometry(visual.outer.Get(), brush_.Get());

                if (visual.inner)
                {
                    brush_->SetColor(visual.fill);
                    render_target_->FillGeometry(visual.inner.Get(), brush_.Get());
                }
