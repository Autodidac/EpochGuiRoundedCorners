...
namespace epochengine::gui_lib::rounded_rect
{
    namespace
    {
        ...
        constexpr std::uint32_t kMinimumSegments = 1;
        constexpr std::uint32_t kMaximumSegments = 64;
    ...
    CornerRadii normalize_corner_radii(Rect bounds, CornerRadii radii) noexcept
    {
        ...
        if (bounds.size.x <= kEpsilon || bounds.size.y <= kEpsilon)
            return {};

        float scale = 1.0f;
        const auto constrain = [&scale](float radius_sum, float extent) noexcept
        {
            if (radius_sum > extent && radius_sum > kEpsilon)
    ...
    RoundedRectMesh make_rounded_rect_mesh(const RoundedRectOptions& options)
    {
        ...
        if (mesh.bounds.size.x <= kEpsilon || mesh.bounds.size.y <= kEpsilon)
            return mesh;
        ...
        if (mesh.border_width > kEpsilon)
        {
            ...
            if (inner_bounds.size.x <= kEpsilon || inner_bounds.size.y <= kEpsilon)
            {
