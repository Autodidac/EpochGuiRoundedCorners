...
namespace epochengine::gui_demo
{
    namespace
    {
        ...
        void draw_text(SceneBatch& batch, float x, float y, std::string_view text, float scale, Color color)
        {
            float cursor = x;
...
void main()
{
    ...
    struct OpenGLRenderer::Impl
    {
        ...
        [[nodiscard]] gui::FloatingWindowOptions window_options(gui::Vec2 position, gui::Vec2 size) const noexcept
        {
            return {
        ...
        [[nodiscard]] int press_target(const input::InputFrame& frame) const noexcept
        {
            ...
            const bool core_visible = display_mode != 1 && core_window.open;
            const bool rounded_visible = display_mode != 0 && rounded_window.open;
            const bool in_core = core_visible
                && gui::contains({ core_window.position, core_window.size }, frame.pointer_position);
            const bool in_rounded = rounded_visible
        ...
        void draw_window_content(gui::Rect content, bool rounded_surface)
        {
            draw_text(batch, content.position.x + 8.0f, content.position.y + 8.0f,
                rounded_surface ? "OPTIONAL ROUNDED GEOMETRY" : "CORE RECTANGULAR FALLBACK",
                1.25f,
                rounded_surface ? green : accent);
            ...
            const gui::ProgressBarLayout progress = gui::make_progress_bar_layout({
                ...
                .value = rounded_surface ? 0.76f : 0.54f,
                .minimum = 0.0f,
                .maximum = 1.0f,
                .padding = 3.0f,
                .direction = gui::ProgressBarDirection::left_to_right
            });
            draw_surface(batch, progress.track, rounded_surface, 7.0f, background, panel_border, 1.0f);
            draw_surface(batch, progress.fill, rounded_surface, 5.0f,
                rounded_surface ? green : accent,
                rounded_surface ? green : accent,
                0.0f);
            ...
            draw_surface(batch, split.before, rounded_surface, 6.0f, background, panel_border, 1.0f);
            draw_surface(batch, split.after, rounded_surface, 6.0f, control_fill, panel_border, 1.0f);
            draw_surface(batch, split.handle, rounded_surface, 3.0f, accent_soft, accent_soft, 0.0f);
            ...
            bool active)
        {
            ...
            draw_surface(batch, layout.window, rounded_surface, 11.0f,
                panel_fill,
                active ? accent : panel_border,
                active ? 2.0f : 1.0f);
            draw_surface(batch, layout.title_bar, rounded_surface, 10.0f,
            ...
            draw_text(batch, layout.title_bar.position.x + 12.0f, layout.title_bar.position.y + 11.0f,
                rounded_surface ? "ROUNDED WINDOW" : "CORE WINDOW",
                1.45f,
                text_primary);
            ...
            draw_window_content(layout.content, rounded_surface);
        ...
        void build_scene()
        {
            ...
            const bool core_active = core_visible
                && (!rounded_visible || core_window.focus_order >= rounded_window.focus_order);
            if (core_active)
            {
            ...
            if (!core_visible && !rounded_visible)
            {
